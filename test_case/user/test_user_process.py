...
#注册
@pytest.mark.realname3#对用例方法标记，执行某几个用例时，输入标记名执行
def test_lin_post_json(pub_data):
    pub_data["username"] = "pytestpytest 字符串 5 数字 linh"
...
#登录
def test_lin_post_login(pub_data):
    method = "POST"  #请求方法，全部大写
    feature = "用户模块"  # allure报告中一级分类
    story = '用户登录'  # allure报告中二级分类
    title = "全字段正常流_1"  # allure报告中用例名字
